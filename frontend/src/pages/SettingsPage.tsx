...
function SettingsPage() {
  ...
  const tabs: { id: SettingsTab; label: string; icon: React.ReactNode }[] = [
    { id: 'system', label: 'System', icon: TabIcons.system },
    { id: 'affinda', label: 'Affinda API', icon: TabIcons.affinda },
    { id: 'webhooks', label: 'Webhooks', icon: TabIcons.webhooks },
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">Configure your Data Nexus Bridge instance</p>
      </div>
        ...
        <div className="border-b border-gray-100">
          <nav className="flex">
            {tabs.map((tab) => (
              ...
              >
                <span className={activeTab === tab.id ? 'text-purple-500' : 'text-gray-400'}>
                  {tab.icon}
                </span>
                {tab.label}
              </button>
        ...
        <div className="p-6">
          ...
          {activeTab === 'system' && (
            <div className="space-y-6">
              {/* Version Information */}
              <div>
                <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Version Information</h3>
                {loadingVersion ? (
                  ...
                  <div className="bg-gray-50 rounded-lg divide-y divide-gray-100">
                    <div className="flex justify-between items-center px-4 py-3">
                      ...
                      <span className="text-sm font-medium text-gray-900">{versionInfo.app_version}</span>
                    </div>
                    {versionInfo.git.is_git_repo && (
                      <>
                        ...
                        <div className="flex justify-between items-center px-4 py-3">
                          ...
                          <span className="text-sm text-gray-900">{formatDate(versionInfo.git.last_commit_date)}</span>
                        </div>
                        {versionInfo.git.last_commit_message && (
                          <div className="px-4 py-3">
                            <span className="text-sm text-gray-600 block mb-1">Last Commit Message</span>
                            <span className="text-sm text-gray-900">{versionInfo.git.last_commit_message}</span>
                          </div>
                        )}
                      </>
                    )}
                    {versionInfo.git.has_uncommitted_changes && (
                      <div className="px-4 py-3">
                        <div className="flex items-center gap-2 text-amber-700 bg-amber-50 px-3 py-2 rounded-lg">
                          ...
                          <span className="text-sm">Uncommitted changes detected</span>
                        </div>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="text-red-500 text-sm">Failed to load version info</div>
                )}
              </div>
              ...
              <div>
                ...
                {loadingStatus ? (
                  ...
                  <div className="bg-gray-50 rounded-lg divide-y divide-gray-100">
                    <div className="flex justify-between items-center px-4 py-3">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${systemStatus.database.status === 'connected' ? 'bg-green-500' : 'bg-red-500'}`} />
                        <span className="text-sm font-medium text-gray-900">{getDatabaseName(systemStatus.database.engine)}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-center px-4 py-3">
                      <span className="text-sm text-gray-600">Available Plugins</span>
                      <span className="text-sm font-medium text-gray-900">{systemStatus.plugins.available}</span>
                    </div>
                    <div className="flex justify-between items-center px-4 py-3">
                      <span className="text-sm text-gray-600">Installed Plugins</span>
                      <span className="text-sm font-medium text-gray-900">{systemStatus.plugins.installed}</span>
                    </div>
                    <div className="flex justify-between items-center px-4 py-3">
                      <span className="text-sm text-gray-600">Active Instances</span>
                      <span className="text-sm font-medium text-gray-900">{systemStatus.plugins.active_instances}</span>
                    </div>
                    <div className="flex justify-between items-center px-4 py-3">
                      <span className="text-sm text-gray-600">Debug Mode</span>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                        systemStatus.debug_mode ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'
                      }`}>
                  ...
                  <div className="text-red-500 text-sm">Failed to load system status</div>
                )}
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              {affindaResult && (
                <div className={`p-4 rounded-lg flex items-center justify-between ${
                  affindaResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
                }`}>
                  <p className={`text-sm ${affindaResult.success ? 'text-green-700' : 'text-red-700'}`}>
                    {affindaResult.message}
                  </p>
                  <button onClick={() => setAffindaResult(null)} className="text-gray-400 hover:text-gray-600">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="bg-blue-50 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-blue-800">
                    <p className="font-medium mb-2">How to find your Affinda credentials</p>
                    <ul className="space-y-2 text-blue-700">
                      <li>
                        <span className="font-medium">API Key:</span> Go to{' '}
                        <a href="https://app.affinda.com/settings/api" target="_blank" rel="noopener noreferrer" className="underline hover:text-blue-900">
                          app.affinda.com/settings/api
                        </a>
              ...
              {loadingAffindaSettings ? (
                ...
                <>
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Current Configuration</h3>
                    <div className="bg-gray-50 rounded-lg divide-y divide-gray-100">
                      <div className="flex justify-between items-center px-4 py-3">
                        <span className="text-sm text-gray-600">API Key Status</span>
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${affindaSettings.is_configured ? 'bg-green-500' : 'bg-red-500'}`} />
                          {affindaSettings.is_configured ? (
                            <span className="font-mono text-xs bg-gray-200 px-2 py-1 rounded text-gray-700">{affindaSettings.api_key}</span>
                          ) : (
                      ...
                      <div className="flex justify-between items-center px-4 py-3">
                        ...
                        <span className="font-mono text-xs text-gray-700">{affindaSettings.base_url}</span>
                      </div>
                      <div className="flex justify-between items-center px-4 py-3">
                        <span className="text-sm text-gray-600">Organization ID</span>
                        <span className="text-sm text-gray-900">
                          {affindaSettings.organization || <span className="text-gray-400 italic">Using environment variable</span>}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Update Settings */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Update Settings</h3>
                        ...
                        {showApiKeyInput ? (
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-4 py-2 text-sm bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 transition"
                            >
                            ...
                            {affindaSettings.api_key_source === 'database' && (
                              <button
                                ...
                                className="px-4 py-2 text-sm bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition disabled:opacity-50"
                              >
                      ...
                      <details className="group">
                        ...
                        <div className="mt-4 pl-5 border-l-2 border-gray-200">
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                          />
                          <p className="text-xs text-gray-500 mt-1">Only change this if using a custom Affinda endpoint</p>
                        </div>
                      </details>
                    </div>
                ...
                <div className="text-red-500 text-sm">Failed to load Affinda settings</div>
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              {webhookResult && (
                <div className={`p-4 rounded-lg flex items-center justify-between ${
                  ...
                  <p className={`text-sm ${webhookResult.success ? 'text-green-700' : 'text-red-700'}`}>
                    {webhookResult.message}
                  </p>
                  <button onClick={() => setWebhookResult(null)} className="text-gray-400 hover:text-gray-600">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
              ...
              {loadingWebhookConfig ? (
                <div className="text-gray-500 text-sm">Loading webhook settings...</div>
              ) : webhookConfig ? (
                <>
                  {/* Setup Instructions */}
                  <div className="bg-blue-50 rounded-lg p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm text-blue-800">
                        <p className="font-medium mb-2">How to configure webhooks</p>
                        <ol className="list-decimal list-inside space-y-1 text-blue-700">
                          ...
                          <li>Select the events to trigger the webhook</li>
                        </ol>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Webhook URL */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Webhook URL</h3>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        readOnly
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-100 transition disabled:opacity-50"
                    ...
                    <p className="text-xs text-gray-500 mt-2">Regenerating the token will invalidate the old URL.</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">When enabled, incoming webhooks will be processed</p>
                    </div>
                    <button
                  ...
                  {/* Event Types */}
                  <div className={!effectiveWebhookEnabled ? 'opacity-50' : ''}>
                    <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-3">Enabled Events</h3>
                    <p className="text-sm text-gray-500 mb-4">
                      {effectiveWebhookEnabled
                        ? 'Select which Affinda events should trigger document syncs'
                        : 'Enable webhooks above to configure events'}
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {webhookConfig.available_events.map((event) => (
                        <label
                          key={event.value}
                          className={`flex items-start gap-3 p-3 bg-gray-50 rounded-lg transition ${
                            effectiveWebhookEnabled
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={effectiveEnabledEvents.includes(event.value)}
                            onChange={() => handleEventToggle(event.value)}
                            disabled={!effectiveWebhookEnabled}
                            className="mt-0.5 h-4 w-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
                          />
                          <div>
                            <span className="font-mono text-xs text-gray-900">{event.value}</span>
                            <p className="text-xs text-gray-500 mt-0.5">{event.label}</p>
                          </div>
                        </label>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-3 pt-6 border-t border-gray-100">
                    ...
                    {hasWebhookChanges && (
                      <button
                        onClick={() => { setPendingWebhookEnabled(null); setPendingEnabledEvents(null); }}
                ...
                <div className="text-red-500 text-sm">Failed to load webhook settings</div>
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
                ...
                <div className={`p-4 rounded-lg ${
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className={`text-sm font-medium ${updateResult.success ? 'text-green-800' : 'text-red-800'}`}>
                    ...
                    <button onClick={() => setUpdateResult(null)} className="text-gray-400 hover:text-gray-600">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Check for and apply available updates</p>
                </div>
                <button
              ...
              {!updateCheck && !checkingUpdates && (
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-gray-500">Click "Check for Updates" to see if a new version is available</p>
                </div>
              )}

              {checkingUpdates && (
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                  <svg className="animate-spin w-8 h-8 mx-auto mb-3 text-purple-600" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                ...
                <div>
                  {updateCheck.error ? (
                    <div className="p-4 bg-red-50 rounded-lg">
                      <p className="text-sm font-medium text-red-800">Error checking for updates</p>
                      <p className="text-sm mt-1 text-red-700">{updateCheck.error}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-blue-700">
                              {updateCheck.commits_behind} commit{updateCheck.commits_behind !== 1 ? 's' : ''} behind
                              {updateCheck.commits_ahead ? ` and ${updateCheck.commits_ahead} commit${updateCheck.commits_ahead !== 1 ? 's' : ''} ahead` : ''}
                            </p>
                          </div>
                        </div>
                        <button
                      ...
                      {updateCheck.new_commits && updateCheck.new_commits.length > 0 && (
                        <div>
                          <h4 className="text-sm font-medium text-gray-700 mb-2">New commits:</h4>
                          <div className="bg-gray-50 rounded-lg p-3 space-y-2 max-h-48 overflow-y-auto">
                            {updateCheck.new_commits.map((commit) => (
                              <div key={commit.hash} className="flex items-start gap-2 text-sm">
                                <span className="font-mono text-xs bg-gray-200 px-1.5 py-0.5 rounded text-gray-600 flex-shrink-0">
                                  {commit.hash}
                                </span>
                    ...
                    <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg">
                      ...
                      <div>
                        <p className="text-sm font-medium text-green-800">You're up to date!</p>
                        <p className="text-xs text-green-700">Latest version on branch {updateCheck.current_branch}</p>
                      </div>
                    </div>
                  )}
                </div>
