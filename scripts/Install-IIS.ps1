...
function Get-InstallConfiguration {
    ...
    $config.AllowedHosts = Read-PromptWithDefault -Prompt "Allowed hosts (comma-separated)" -Default "localhost,127.0.0.1,.localhost,$($config.HostHeader)"
...
function Build-Frontend {
    ...
    try {
        # Create frontend .env file to set API URL to relative path (same origin)
        $frontendEnvPath = Join-Path $frontendPath ".env.production"
        Set-Content -Path $frontendEnvPath -Value "VITE_API_BASE_URL="
        Write-Info "Created frontend .env.production with relative API URL"
...
# Django settings
DJANGO_DEBUG=$($Config.Debug.ToString().ToLower())
SECRET_KEY=$($Config.SecretKey)
DJANGO_ALLOWED_HOSTS=$($Config.AllowedHosts)
...
function Install-IISSite {
    ...
    $wsgiPath = Join-Path $InstallPath "data_nexus_bridge_service\wsgi.py"
...
<configuration>
    <appSettings>
        ...
        <add key="WSGI_HANDLER" value="data_nexus_bridge_service.wsgi.application" />
        <add key="DJANGO_SETTINGS_MODULE" value="data_nexus_bridge_service.settings" />
    </appSettings>
    <system.webServer>
        <handlers>
...
"@
    ...
    # Patch urls.py to serve frontend assets and SPA routes
    Write-Info "Patching Django URLs to serve frontend..."
    $urlsPath = Join-Path $InstallPath "data_nexus_bridge_service\urls.py"
    $urlsContent = @'
"""
URL configuration for data_nexus_bridge_service project.
"""
...
def serve_vite_svg(request):
    file_path = os.path.join(settings.BASE_DIR, 'frontend', 'dist', 'vite.svg')
    if os.path.exists(file_path):
...
urlpatterns = [
    ...
    path('vite.svg', serve_vite_svg, name='vite-svg'),
    path('', include('affinda_bridge.urls')),
    path('', include('plugins.urls')),
    path('', serve_frontend, name='frontend-home'),
]
'@
    Set-Content -Path $urlsPath -Value $urlsContent -Encoding UTF8
    Write-Success "Django URLs patched for frontend serving"
...
function New-DjangoSuperuser {
    ...
    try {
        ...
        # Write batch file with environment variables set (line by line to avoid here-string issues)
        $batchLines = @(
            "@echo off",
        ...
        Set-Content -Path $batchFile -Value ($batchLines -join "`r`n")
